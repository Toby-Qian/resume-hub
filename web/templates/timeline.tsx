...
export default function Timeline({ resume }: TemplateProps) {
  ...
  const work = resume.work.length > 0 && (
    <>
      ...
      {resume.work.map((w, i) => (
        <Node key={w.id} date={<DateRange startPath={`work.${i}.startDate`} endPath={`work.${i}.endDate`} start={w.startDate} end={w.endDate} />} breakBefore={w.breakBefore}>
  ...
  const projects = resume.projects.length > 0 && (
    <>
      ...
      {resume.projects.map((p, i) => (
        <Node key={p.id} date={<DateRange startPath={`projects.${i}.startDate`} endPath={`projects.${i}.endDate`} start={p.startDate} end={p.endDate} />} breakBefore={p.breakBefore}>
  ...
  const education = resume.education.length > 0 && (
    <>
      ...
      {resume.education.map((e, i) => (
        <Node key={e.id} date={<DateRange startPath={`education.${i}.startDate`} endPath={`education.${i}.endDate`} start={e.startDate} end={e.endDate} />} breakBefore={e.breakBefore}>
