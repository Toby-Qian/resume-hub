...
export default function AcademicClassic({ resume }: TemplateProps) {
      ...
      {resume.projects.map((p, idx) => (
        <div key={p.id} className={itemCls(p, "mb-2 text-[0.92em]")}>
          <div>
            ...
            {(p.startDate || p.endDate) && <span className="text-gray-600"> ({<DateRange startPath={`projects.${idx}.startDate`} endPath={`projects.${idx}.endDate`} start={p.startDate} end={p.endDate} />})</span>}
          </div>
  ...
  const ordered = useOrderedSections({ work, education, projects, skills, awards, languages, publications, talks, teaching });
