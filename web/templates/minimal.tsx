...
export default function Minimal({ resume }: TemplateProps) {
  ...
  const Row = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <section className="grid grid-cols-[110px_1fr] gap-4 mb-5">
  ...
  return (
    <div style={{ padding: "var(--pad)" }}>
      ...
      {resume.education.length > 0 && (
        <Row title="Edu">
          {resume.education.map((e) => (
            <div key={e.id} className="text-[0.9em]">
      ...
      {resume.projects.length > 0 && (
        <Row title="Projects">
          {resume.projects.map((p) => (
            <div key={p.id} className="text-[0.9em]">
      ...
      {resume.skills.length > 0 && (
        <Row title="Skills">
          {resume.skills.map((s) => (
            <div key={s.id} className="text-[0.9em]"><b>{s.name}</b> — {s.keywords.join(", ")}</div>
          ))}
        </Row>
      )}
      ...
      {resume.awards.length > 0 && (
        <Row title="Awards">
          {resume.awards.map((a) => (
            <div key={a.id} className="text-[0.9em]"><b>{a.title}</b> — {a.awarder} <span className="text-gray-400">{a.date}</span></div>
          ))}
        </Row>
      )}
